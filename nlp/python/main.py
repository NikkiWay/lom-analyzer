...
def warmup(x_auth_token: str = Header(...)):
    """Загружает модели заранее и сообщает, что удалось загрузить.

    Модели грузятся при первом обращении к эндпоинту, и холодная загрузка занимает
    больше, чем таймаут рабочего запроса. Без прогрева её ждёт первый же рабочий
    вызов и отваливается по таймауту, а вызывающая сторона считает модель
    недоступной.

    Каждая модель грузится независимо: отказ одной оставляет остальные загруженными.

    Возвращает {'models': {имя: версия}, 'failed': {имя: причина}}. Версии
    записываются в сессию и показывают, чем она посчитана.
    """
    verify_token(x_auth_token)
    loaders = {
