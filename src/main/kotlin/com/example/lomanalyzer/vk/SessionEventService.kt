...
class SessionEventService(
     ...
     *
     * Тип отдельный, а не текстовое предупреждение: по нему этап проверки качества
     * определяет, что сессия считалась урезанным фильтром, и проваливает
     * соответствующий индикатор. Сессия при этом завершается успешно, поэтому по
     * её статусу такой прогон от полноценного не отличить.
     *
     * @param reason причина отключения (показывается аналитику).
     */
