 ...
 * findBySession/findBySessionAndPost (выборки, отсортированы по времени
 * публикации), countBySession/countByPost (агрегатные COUNT),
 * countBySessionGroupedByPost (счётчики по всем постам сессии одним запросом).
 *
 * БИБЛИОТЕКИ
...
class CommentDao(private val db: Database) {
     ...
     *
     * Условие начинается с session_id, ведущей колонки индекса
     * (session_id, post_id), поэтому выборка идёт поиском по индексу, а
     * группировка даёт все счётчики за один проход. Для подсчёта по множеству
     * постов следует использовать этот метод, а не countByPost в цикле:
     * countByPost фильтрует только по post_id, ведущей колонки в условии не
     * имеет и опереться на индекс не может.
     *
