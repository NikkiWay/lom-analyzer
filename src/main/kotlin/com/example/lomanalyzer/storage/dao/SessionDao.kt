...
class SessionDao(private val db: Database) {
    ...
    /**
     * UPDATE фактического режима NLP и версий моделей. Вызывается после
     * инициализации NLP, когда известно, что удалось поднять.
     *
     * При создании сессии в nlp_mode попадает значение из параметров, то есть
     * намерение; фактический режим зависит от того, поднялся ли sidecar, и
     * известен только здесь. Без этой записи сессию, посчитанную словарём, не
     * отличить от посчитанной моделью.
     *
     * @param mode фактический режим: FULL или FALLBACK_ONLY.
     * @param modelVersions версии использованных моделей (JSON).
     */
