...
class SentimentResultDao(private val db: Database) {
     ...
     * посты и комментарии нумеруются независимо, и общая карта по обоим типам
     * теряла бы строки при совпадении идентификаторов.
     ...
     *
     * Используется проверкой качества: по методу видно, измерена ли тональность
     * моделью или подставлена после сбоя (fallback_error). Ограничение по типу
     * сущности — как в [findAllAsMap].
     *
     * @param entityType тип сущности, по которому строится карта.
