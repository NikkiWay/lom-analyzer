 ...
 *
 * Подобран по фактическому разбросу склонностей: на сессии из 86 авторов
 * |positive − negative| имеет медиану 0.065, p75 = 0.10, p90 = 0.18 и максимум
 * 0.59. Модель тональности сдержанна к спокойной прозе, поэтому почти весь
 * разброс лежит в нижней десятой части шкалы [0, 1], и линейная насыщенность
 * оставила бы его неразличимым.
 *
 * Кубический корень раскрывает именно этот участок: 0.065 даёт насыщенность
 * 0.40 против 0.25 у квадратного корня, и заметный цвет получают 58 авторов
 * из 86 вместо 17. Линейную шкалу с отсечкой (например, |score| / 0.15) не
 * берём: она схлопывает верх диапазона, и склонность 0.59 стала бы неотличима
 * от 0.15.
 */
