...
> **Автоматический сбор через VK API временно недоступен** — изменились правила доступа к API. Весь остальной путь работает: данные загружаются готовым JSON на экране «Настройка», и пайплайн проходит целиком. Демонстрационный набор и пример заполнения — в [`examples/`](examples/).
...
Ниже — прогон на демонстрационном наборе [`examples/dataset_demo.json`](examples/dataset_demo.json); его авторы и сообщества вымышлены.

### Дашборд

Диаграмма квадрантов (Struct_a × Topic_a) с линиями порогов и таблица всех авторов с 11 оценками и ролью. Цвет точки — позиция автора, размер — вовлечённость по теме. Справа — справочник метрик с формулами.
...
Отбор публикаций по теме с разбивкой по источникам и ручной проверкой решений фильтра.

![Валидация темы](docs/screenshots/topic-validation.png)

### Качество сессии и журнал

Индикаторы качества отвечают на вопрос, можно ли доверять числам: часть сбоев не прерывает прогон, и сессия завершается успешно, но считается более грубым способом. Журнал показывает хронику прогона целиком.
...
### Руководство и профиль

Встроенная справка по работе и методу; профиль — управление токеном, мастер-паролем, сессиями и данными.

<table>
<tr>
<td width="50%"><img src="docs/screenshots/guide.png" alt="Руководство пользователя"></td>
<td width="50%"><img src="docs/screenshots/profile.png" alt="Профиль"></td>
</tr>
</table>
...
В [`examples/`](examples/) лежат пример заполнения сессии и демонстрационный набор данных. Набор загружается на экране «Настройка» через импорт JSON: пайплайн проходит целиком, обращения к VK не требуется.
...
- Авторы и сообщества в демонстрационном наборе `examples/dataset_demo.json` вымышлены.

## Известные ограничения

- **Автоматический сбор через VK API временно недоступен**: правила доступа к API изменились. Работа с данными идёт через импорт готового JSON — весь пайплайн от предобработки до ролей проходит целиком, минуя только стадию сбора.
